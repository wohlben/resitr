...
export class UserWorkoutLogService {
  ...
  async listLogs(userId: string, workoutTemplateId?: string) {
    let logs: Awaited<ReturnType<typeof this.logRepository.findByUserId>>;

    if (workoutTemplateId) {
      logs = await this.logRepository.findByUserIdAndWorkoutTemplateId(userId, workoutTemplateId);
    } else {
      logs = await this.logRepository.findByUserId(userId);
    }
        ...
        for (const item of section.items) {
          if (item.setIds.includes(setId)) {
            ...
            if (allSetsDone) {
              ...
              const allItemsDone = section.items.every((i) => i.id === item.id || i.completedAt);
              if (allItemsDone) {
                await this.logSectionRepository.update(section.id, { completedAt: new Date() });

                const allSectionsDone = log.sections.every((s) => s.id === section.id || s.completedAt);
                if (allSectionsDone) {
  ...
  private async populateSectionsItemsSets<T extends { sectionIds: string[] }>(logs: T[]) {
    ...
    const sections = allSectionIds.length > 0 ? await this.logSectionRepository.findByIds(allSectionIds) : [];
    const sectionsById = new Map(sections.map((s) => [s.id, s]));

    // Batch fetch all items
    const allItemIds = sections.flatMap((s) => s.itemIds);
    const items = allItemIds.length > 0 ? await this.logItemRepository.findByIds(allItemIds) : [];
    const itemsById = new Map(items.map((i) => [i.id, i]));

    // Batch fetch all sets
    const allSetIds = items.flatMap((i) => i.setIds);
    const sets = allSetIds.length > 0 ? await this.logSetRepository.findByIds(allSetIds) : [];
