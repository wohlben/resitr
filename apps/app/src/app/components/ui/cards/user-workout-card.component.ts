...
export type UserWorkoutAction = 'viewLogs' | 'schedule' | 'delete';
  ...
  template: `
    <app-card>
      ...
      @if (userWorkout().workout) {
        ...
        >
          {{ userWorkout().workout!.sections.length }} section{{ userWorkout().workout!.sections.length !== 1 ? 's' : '' }}
        </span>
      }
